# Future is Near - Smart Home Landing Page

This project is a minimalist landing page for a fictional smart home company called **future is near.** It was developed as a final project for a Technical Computer Science High School.

## Key Features
* **Interactive SVG Animation:** A swinging light bulb that can be turned on/off using a wall switch or a simulated smartphone interface.
* **Animated Menu:** A dynamic, full-screen navigation menu.
* **Desktop Only:** Optimized strictly for desktop viewports (not responsive).

## Tech Stack
* **HTML5 & CSS3** (Animations via SVG and @keyframes).
* **jQuery** (Interaction logic and DOM manipulation).
* **SVG** (Vector graphics used for all primary interactive elements and animations).

## How to run
1. Download the repository.
2. Open the `index.html` file in any modern desktop browser.

---
*Created by Natan Mokrzycki*
